...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(0, 0, 0);
		text("Size - Magnitude", 75, 125);
		text("Land Quakes", 75, 175);
		text("Ocean Quakes", 75, 225);
		text("Cities", 75, 275);
		text("Shallow", 75, 325);
		text("Intermediate", 75, 350);
		text("Deep", 75, 375);
	}
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			boolean check = isInCountry(earthquake,m);
			if(check){
				return true;
			}
		}
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			...
			for (Marker m : quakeMarkers) {
					...
					boolean check = ((EarthquakeMarker) m).isOnLand();
					if(check == false){
						quakeOcean++;
						}
					}
			if(quakeCounter > 0){
			System.out.println("The country " + (String) cm.getProperty("name") + " had " + 
		    quakeCounter + " quakes"); 
			}
		}
			System.out.println("There were " + (quakeOcean/countryMarkers.size()) + 
					" quakes in the ocean");
